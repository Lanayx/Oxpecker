...
module internal ReflectionBasedModelParser =
    ...
    and getValueForComplexType
        ...
        if tryResolveComplexType then
            ...
            | true ->
                ...
                let res = parseModel model culture dictData
                match res with
                | Ok(NonNull o) -> prop.PropertyType.MakeSomeCase(o) |> Option.ofObj
                | Ok Null
                | Error _ -> None
        else
            None
    ...
    and getValueForArrayOfGenericType
        ...
        if prop.PropertyType.IsArray then
            ...
            let innerType = prop.PropertyType.GetElementType() |> nonNull
...
type ModelBinding() =
    ...
    static let reflectionBasedModelBinder =
