...
module TestApp =
    ...
    let app =
        [
            ...
            Route.Authenticated, mustBeLoggedIn >=> ok Response.Authenticated
            Route.AdminOnly, mustBeAdmin >=> ok Response.AdminOnly
            Route.AdminOrOperator, mustBeOperatorOrAdmin >=> ok Response.AdminOrOperator
            Route.JohnOnly, mustBeJohn >=> ok Response.JohnOnly
        ]
        |> dict
...
module TestData =
    ...
    let mkAdmin name = [ Claim(ClaimTypes.Name, name); Claim(ClaimTypes.Role, "admin") ]

    let mkOperator name = [ Claim(ClaimTypes.Name, name); Claim(ClaimTypes.Role, "operator") ]

let testAuthentication (givenUser: User) givenRoute expected =
    task {
        let ctx = DefaultHttpContext()
        ctx.Response.Body <- new MemoryStream()
        let handler = app[givenRoute]
        ...
        ctx.Response.StatusCode |> shouldEqual expectedStatusCode
        ctx.Response.Body.Seek(0, SeekOrigin.Begin) |> ignore
        use reader = new StreamReader(ctx.Response.Body)
        reader.ReadToEnd() |> shouldEqual expectedContent
    }
