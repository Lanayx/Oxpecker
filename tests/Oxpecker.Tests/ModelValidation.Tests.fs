...
let ``Valid model doesn't raise any issues`` () =
    let model = { Name = "John"; Age = 30; Email = "my@email.com"; Active = true }
    ...
    | ModelValidationResult.Invalid _ ->
        failwith "Expected valid model"


[<Fact>]
let ``Invalid model raises issues`` () =
    let model = { Name = null; Age = 200; Email = "abc"; Active = false }
    let result = validateModel model
    ...
    | ModelValidationResult.Invalid (x, errors) ->
        ...
        errors.ErrorMessagesFor(nameof x.Age) |> Seq.head |> shouldEqual "The field Age must be between 0 and 100."
        errors.ErrorMessagesFor(nameof x.Email) |> Seq.toList |> shouldEqual [
    ...
    | _ ->
        failwith "Expected invalid model"
